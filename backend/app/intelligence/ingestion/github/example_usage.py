...
instructions) to sanity-check the scanner end to end.
    ...
    python -m app.ingestion.github.example_usage /path/to/repo <since_commit>
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python -m app.ingestion.github.example_usage <repo_path> <since_commit> [--no-classify]")
        sys.exit(1)
    ...
    for r in results:
        flag = "DATA-HANDLING" if r.get("is_data_handling") else "flagged (unconfirmed)"
        print(f"[{flag}] {r['file_path']}:{r['line_number']}  {r['content']}")
        if classify:
            print(f"    -> data_type={r.get('data_type')} vendor={r.get('vendor')} "
