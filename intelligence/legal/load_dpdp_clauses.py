...
from legal.dpdp_extractor import BATCH_SIZE, REQUESTS_PER_MINUTE, DPDPClauseExtractor
...
def main():
    ...
    print(f"Extraction will cost ~{n_calls} Gemini API calls "
          f"(batches of {BATCH_SIZE}), ~{est_minutes:.1f} min at {REQUESTS_PER_MINUTE} RPM.")

    if not args.yes:
        answer = input("Proceed with extraction and graph write? [y/N] ").strip().lower()
        if answer != "y":
    ...
    if needs_review:
        print(f"{len(needs_review)} sections need manual review (extraction failed).")

    if args.min_confidence > 0.0:
        before = len(governing)
        governing = [e for e in governing if (e.get("confidence") or 0.0) >= args.min_confidence]
        ...
        extracted = [e for e in extracted if e.get("is_data_governing") is not True or e in governing]
